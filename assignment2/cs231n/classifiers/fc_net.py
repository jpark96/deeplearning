...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        a2, a2_cache = affine_forward(z1, W2, b2)
        scores = a2
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dz0, dW1, db1 = affine_backward(da1, a1_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(dim_list) - 1):
            ...
            bias_str = 'b' + str(i + 1)
            self.params[weight_str] = weight_scale * np.random.randn(dim_list[i], dim_list[i + 1])
            self.params[bias_str] = np.zeros(dim_list[i + 1])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(len(dim_list) - 1):
            weight_str = 'W' + str(i + 1)
            bias_str = 'b' + str(i + 1)
            W = self.params[weight_str]
            b = self.params[bias_str]
            out, affine_cache = affine_forward(out, W, b)
            affine_cache_lst.append(affine_cache)
            
            loss_reg_term += np.sum(W * W)
        ...
        #find grad of last affine layer
        weight_str = 'W' + str(len(dim_list) - 1)
        bias_str = 'b' + str(len(dim_list) - 1)
        affine_cache = affine_cache_lst.pop()
        dloss, dweight, dbias = affine_backward(dloss, affine_cache)
        dweight += reg * affine_cache[1]
